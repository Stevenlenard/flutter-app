...
class _AdminDashboardState extends State<AdminDashboard> {
    ...
    _database.ref('system_logs').limitToLast(10).onValue.listen((event) {
      if (event.snapshot.exists && event.snapshot.value != null) {
        ...
        data.forEach((key, value) {
          if (value is Map) {
            logs.add(Map<dynamic, dynamic>.from(value));
          }
        });
  ...
  Future<void> _checkAutoBackupDownload() async {
    try {
      ...
      if (settingsData is Map && 
          settingsData['success'] == true && 
          settingsData['data'] != null &&
          settingsData['data']['auto_backup'] == true) {
            ...
            if (await canLaunchUrl(uri)) {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text("Daily auto-backup downloaded: ${data['filename']}")),
                );
              }
            }
          }
  ...
  void _setupListeners() {
      ...
      if (event.snapshot.exists && event.snapshot.value != null) {
        ...
        data.forEach((key, value) {
          if (value != null && value is Map) {
            // Check if the route is for today (Aligned with integration_spec.md)
            final String? routeDate = value['date']?.toString();
            if (routeDate == today) {
              total++;
              // Check route_status per spec, fallback to status
      ...
      if (event.snapshot.exists && event.snapshot.value != null) {
        ...
        if (mounted) {
          setState(() {
            ...
            _inProgressComplaints = inProgress;
    ...
    _database.ref('notification_logs').onChildAdded.listen((event) async {
      if (event.snapshot.exists && event.snapshot.value != null) {
        ...
        // Check global admin setting from Firebase-synced service
        final bool isGlobalEnabled = _adminSettingsService.appNotificationsEnabled.value;
        // Also check local session as fallback/double-check
        ...
        if (mounted) {
          setState(() {
            ...
            _resolvedComplaints = complaints
                .where((c) => normalize(c['status']) == 'RESOLVED')
                .length;
            ...
            // Calculate Hotspots (Areas with > 3 pending complaints)
            final Map<String, int> areaComplaints = {};
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        bool isMobile = constraints.maxWidth < 900; // Increased threshold for desktop layout

        return Scaffold(
          backgroundColor: const Color(0xFFF0F2F5), // More modern light gray
  ...
  Widget _buildSidebar() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        boxShadow: [
          BoxShadow(color: Colors.black.withAlpha(5), blurRadius: 10, offset: const Offset(5, 0)),
        ],
      ),
      child: Column(
        children: [
          const SizedBox(height: 24),
          _isSidebarExtended ? Padding(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.local_shipping_rounded, color: AppColors.tealText),
                ),
                const SizedBox(width: 12),
                const Text("G-Tracker", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 20, color: AppColors.tealText)),
              ],
            ),
          ) : const Icon(Icons.local_shipping_rounded, color: AppColors.tealText, size: 32),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                _buildSidebarItem(Icons.map_rounded, "Track Trucks", 1),
                _buildSidebarItem(Icons.analytics_rounded, "Analytics", 2),
                _buildSidebarItem(Icons.chat_bubble_rounded, "Complaints", 3),
                _buildSidebarItem(Icons.people_outline_rounded, "Users", 4),
                _buildSidebarItem(Icons.storage_rounded, "Data Management", 5),
                _buildSidebarItem(Icons.settings_suggest_rounded, "Settings", 6),
              ],
            ),
          ),
          IconButton(
            icon: Icon(_isSidebarExtended ? Icons.arrow_back_ios_rounded : Icons.arrow_forward_ios_rounded, size: 16),
            onPressed: () => setState(() => _isSidebarExtended = !_isSidebarExtended),
          ),
          const SizedBox(height: 16),
          Padding(
            padding: const EdgeInsets.only(bottom: 24),
            child: _buildSidebarItem(Icons.logout_rounded, "Logout", -1, isLogout: true),
          ),
        ],
      ),
    );
          ...
          children: [
            ...
            if (_isSidebarExtended) ...[
              ...
              Text(
                label,
                style: TextStyle(
                  color: isLogout ? Colors.red : (isSelected ? AppColors.tealText : Colors.grey.shade700),
      ...
      child: Column(
        children: [
          UserAccountsDrawerHeader(
            ...
            accountName: const Text("System Admin", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
            accountEmail: const Text("Barangay Balintawak, Lipa City"),
            currentAccountPicture: CircleAvatar(
              backgroundColor: Colors.white,
              child: const Icon(Icons.person, color: AppColors.tealText, size: 40),
            ),
          ),
          _buildDrawerItem(Icons.person_add_alt_1_rounded, "User Management", () {
  ...
  Widget _buildDrawerItem(IconData icon, String title, VoidCallback onTap, {Color? color}) {
    return ListTile(
      ...
      title: Text(title, style: TextStyle(fontWeight: FontWeight.w700, color: color ?? Colors.grey.shade800)),
      onTap: onTap,
    );
  }

  Widget _buildMainDashboard(bool isMobile) {
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          "Overview of Barangay Balintawak G-Tracker",
                          style: TextStyle(
                            fontSize: 14,
                    ...
                    if (!isMobile) Row(
                      children: [
                        ...
                        const CircleAvatar(
                          ...
                          child: Icon(Icons.person, color: AppColors.tealText),
                        ),
                      ],
                    ),
                  ],
            ...
            children: [
              ...
              if (badge != null)
                Container(
                  ...
                  child: Text(badge, style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 12)),
                ),
            ],
          ),
          Column(
                ...
                Row(
                  children: [
                    ...
                    const Text(
                      ...
                      style: TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A), fontSize: 18),
                    ),
                  ],
                ),
                TextButton.icon(
                  ...
                  label: const Text("FULL MAP", style: TextStyle(fontWeight: FontWeight.w900)),
        ...
        children: [
          const Row(
            children: [
              ...
              SizedBox(width: 12),
              Text("Recent Activity", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A))),
            ],
          ),
          const SizedBox(height: 20),
          if (_recentLogs.isEmpty)
            const Center(child: Text("No recent activity logs", style: TextStyle(color: Colors.grey, fontSize: 13)))
          else
            Column(
              children: _recentLogs.take(5).map((log) => _buildRecentActivityItem(log)).toList(),
            ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.local_shipping_rounded, color: Color(0xFF1976D2), size: 24),
              SizedBox(width: 12),
              Text("Fleet Overview", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A))),
            ],
          ),
          const SizedBox(height: 20),
          ...
          SizedBox(
            ...
            child: TextButton(
              onPressed: () => setState(() => _selectedIndex = 1),
              child: const Text("VIEW ALL FLEET", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.1)),
            ),
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.insights_rounded, color: Color(0xFF009688), size: 24),
              SizedBox(width: 12),
              Text("Performance", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A))),
            ],
          ),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 12),
          _buildModernInfoRow("Active Hotspots", "$_activeHotspots Areas"),
        ],
      ),
    );
  }
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(label, style: TextStyle(color: Colors.grey.shade600, fontSize: 13, fontWeight: FontWeight.w700)),
  ...
  Widget _buildBottomNav() {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        boxShadow: [BoxShadow(color: Colors.black.withAlpha(5), blurRadius: 25, offset: const Offset(0, -5))],
      ),
      child: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.map_rounded), label: 'Track'),
          BottomNavigationBarItem(icon: Icon(Icons.analytics_rounded), label: 'Analytics'),
          BottomNavigationBarItem(icon: Icon(Icons.chat_bubble_rounded), label: 'Issues'),
          BottomNavigationBarItem(icon: Icon(Icons.people_outline_rounded), label: 'Users'),
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  const Expanded(child: Text("Notifications", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A)))),
                  TextButton(
                    onPressed: () async {
                      await _database.ref('notification_logs').remove();
                      if (mounted) Navigator.pop(context);
                    },
                    child: const Text("Clear All", style: TextStyle(color: Color(0xFF00BFA5), fontWeight: FontWeight.w900)),
                  ),
                ],
              ),
              const SizedBox(height: 24),
              Flexible(
                child: StreamBuilder(
                  stream: _database.ref('notification_logs').onValue,
                  builder: (context, snapshot) {
                    if (snapshot.hasData && snapshot.data!.snapshot.exists) {
                      final Map? data = snapshot.data!.snapshot.value as Map?;
                      if (data == null) return const Center(child: Text("No notifications"));
                      final List list = [];
                      data.forEach((k, v) => list.add({...v as Map, 'id': k}));
                      list.sort((a, b) => (b['timestamp'] ?? 0).compareTo(a['timestamp'] ?? 0));
                      return ListView.separated(
                        shrinkWrap: true,
                        itemCount: list.length,
                        separatorBuilder: (c, i) => const SizedBox(height: 12),
                        itemBuilder: (c, i) => _buildNotificationItem(list[i]),
                      );
                    }
              ...
              ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1A1A1A), foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 56), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
  ...
  Widget _buildNotificationItem(Map item) {
    ...
    return InkWell(
      onTap: () => _database.ref('notification_logs/${item['id']}').update({'isRead': true}),
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: isRead ? Colors.white : const Color(0xFFF0FDF4),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: isRead ? Colors.grey.shade100 : const Color(0xFFDCFCE7)),
        ),
        child: Row(
          children: [
            Container(
              ...
              child: Icon(item['type'] == 'DRIVER_ISSUE' ? Icons.error_rounded : Icons.notifications_rounded, color: item['type'] == 'DRIVER_ISSUE' ? Colors.red : Colors.teal, size: 20),
            ),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(item['title'] ?? 'Alert', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 14)),
                  Text(item['message'] ?? '', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                ],
              ),
            ),
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              const Text("Sign Out?", style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900)),
              const SizedBox(height: 12),
              Text("Are you sure you want to exit?", style: TextStyle(color: Colors.grey.shade600)),
              const SizedBox(height: 32),
              ElevatedButton(
                onPressed: () async {
