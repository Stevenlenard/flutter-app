...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() async {
    ...
    try {
      ...
      if (data is Map && data['success'] == true) {
        ...
        if (role == 'admin') Navigator.pushReplacementNamed(context, '/admin_dashboard');
        else if (role == 'resident') Navigator.pushReplacementNamed(context, '/resident_dashboard');
        else if (role == 'driver') Navigator.pushReplacementNamed(context, '/driver_dashboard');
      } else {
                                ...
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      boxShadow: [BoxShadow(color: AppColors.loginButtonEnd.withAlpha(100), blurRadius: 20, offset: const Offset(0, 10))],
                                    ),
                                    child: const Icon(Icons.local_shipping_rounded, size: 54, color: Colors.white),
                                  ),
                                  const SizedBox(height: 32),
                                  const Text('Garbage Tracker', style: TextStyle(fontSize: 36, fontWeight: FontWeight.w900, color: AppColors.tealText, letterSpacing: -1.5, height: 1.1)),
                                  const SizedBox(height: 8),
                                  const Text('Brgy. Balintawak, Lipa City', style: TextStyle(fontSize: 15, color: AppColors.textGray, fontWeight: FontWeight.w600, letterSpacing: 0.5)),
                                  const SizedBox(height: 48),
                                  _buildRefinedTextField(label: 'Username', hint: 'Enter your username', controller: _usernameController, icon: Icons.person_outline_rounded, validator: (v) => (v == null || v.isEmpty) ? 'Please enter your username' : null),
                                  const SizedBox(height: 24),
                                  _buildRefinedTextField(label: 'Password', hint: 'Enter your password', controller: _passwordController, isPassword: true, obscureText: _obscurePassword, icon: Icons.lock_outline_rounded, onTogglePassword: () => setState(() => _obscurePassword = !_obscurePassword), validator: (v) => (v == null || v.isEmpty) ? 'Please enter your password' : (v.length < 6 ? 'Password must be at least 6 characters' : null)),
                                  Align(
                                    alignment: Alignment.centerRight,
                                    child: TextButton(onPressed: () => Navigator.pushNamed(context, '/forgot_password'), child: const Text('Forgot Password?', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14))),
                                  ),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      const Text("Don't have an account? ", style: TextStyle(color: AppColors.textGray, fontSize: 14)),
                                      GestureDetector(onTap: () => Navigator.pushNamed(context, '/register'), child: const Text('Create Account', style: TextStyle(color: AppColors.tealLink, fontWeight: FontWeight.w900, fontSize: 14))),
                                    ],
                                  ),
                                ],
                              ),
  ...
  Widget _buildWarningCard(String msg, Color color) {
    return Container(
      ...
      child: Row(children: [Icon(Icons.warning_amber_rounded, color: color, size: 18), const SizedBox(width: 8), Expanded(child: Text(msg, style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.w600)))]),
    );
  }
  ...
  Widget _buildLockoutCard() {
    return Container(
      ...
      child: Column(children: [
        Row(children: [Icon(Icons.lock_clock_rounded, color: Colors.red.shade900, size: 20), const SizedBox(width: 12), Expanded(child: Text('Too many failed login attempts.', style: TextStyle(color: Colors.red.shade900, fontWeight: FontWeight.bold)))]),
        const SizedBox(height: 12),
        ...
        const SizedBox(height: 12),
        TextButton.icon(onPressed: () => Navigator.pushNamed(context, '/forgot_password'), icon: const Icon(Icons.refresh_rounded, size: 18), label: const Text('Reset your password'), style: TextButton.styleFrom(foregroundColor: Colors.red.shade900)),
      ]),
    );
  }
    ...
    return Padding(
      ...
      child: Column(children: [
        Wrap(alignment: WrapAlignment.center, spacing: 12, children: [
          GestureDetector(onTap: () => LegalAgreementDialog.show(context, isTerms: true), child: const Text('Terms & Conditions', style: TextStyle(color: AppColors.tealLink, fontSize: 12, fontWeight: FontWeight.bold, decoration: TextDecoration.underline))),
          const Text('•', style: TextStyle(color: AppColors.textGray)),
          GestureDetector(onTap: () => LegalAgreementDialog.show(context, isTerms: false), child: const Text('Privacy Policy', style: TextStyle(color: AppColors.tealLink, fontSize: 12, fontWeight: FontWeight.bold, decoration: TextDecoration.underline))),
        ]),
        const SizedBox(height: 12),
  ...
  Widget _buildRefinedTextField({required String label, required String hint, required TextEditingController controller, IconData? icon, bool isPassword = false, bool obscureText = false, VoidCallback? onTogglePassword, String? Function(String?)? validator}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextFormField(
        controller: controller, obscureText: isPassword ? obscureText : false, validator: validator,
        ...
        decoration: InputDecoration(
          ...
          contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 20), filled: true, fillColor: Colors.grey.shade50,
  ...
  Widget _buildAnimatedLoginButton() {
    ...
    return GestureDetector(
      ...
      child: AnimatedScale(
        scale: _buttonScale, duration: const Duration(milliseconds: 100),
        child: Container(
          width: double.infinity, height: 60,
          decoration: isLocked ? AppDecorations.loginButton.copyWith(gradient: LinearGradient(colors: [Colors.grey.shade400, Colors.grey.shade500]), boxShadow: []) : AppDecorations.loginButton,
          alignment: Alignment.center,
          child: _isLoading ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.5)) : Text(isLocked ? 'Locked' : 'Sign In', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 1)),
        ),
      ),
    );
  }
